        ...
        for (let ix = 0; ix < consDots; ix++) {
            for (let iy = 0; iy < consDots; iy++) {
                ...
                // ТВОЙ НОВЫЙ БЛОК:
                    ...
                    // Идеально сглаженная матрица 40x26 (с наращенным круглым куполом сверху)
                    const grid = [
                        "             ##############             ", // 0 (Наращенная округлая макушка)
                        "          ####------------####          ", // 1 (Плавный спуск)
                        "        ###------------------###        ", // 2 (Плавный спуск)
                        "      ##------------------------##      ", // 3 (Идеальное слияние с контуром)
                        "    ##------####--------####------##    ", // 4 (Начало очков)
                        "   ##----#####------------#####----##   ", // 5
                        "  ##----####----------------####----##  ", // 6
                        "  ##---####------------------####---##  ", // 7
                    ...
                    // Фиксированный масштаб для сохранения пропорций
                    let pixelSizeX = 0.045; 
                    let pixelSizeY = 0.058; 
                    
                    // КВАНТОВАННАЯ АНИМАЦИЯ (чтобы края не дрожали и лицо не растягивалось)
                    let bounceAmplitude = Math.abs(Math.sin(t * 2.5)) * 0.1; 
                    let rowOffset = Math.floor(bounceAmplitude / pixelSizeY); // Прыжок ровно на 1 пиксель вверх
                    
                    // Центрирование и выборка с учетом жесткого сдвига сетки
                    let col = Math.floor((nx / pixelSizeX) + 20);
                    let row = Math.floor((ny / pixelSizeY) + 13) - rowOffset;
                    ...
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        ...
                        if (char === ' ') {
                            // Блики могут быть ТОЛЬКО внутри очков
                            let isGlare = (row === 10 || row === 11) && ((col >= 10 && col <= 11) || (col >= 28 && col <= 29));
                            if (isGlare) {
                                alpha = 1.0; // Глаза (ярко-белые)
                            }
                        } else if (char === '#') {
                            // Зубы или контур
                            ...
                            if (isTeeth) {
                                ...
                                alpha = 0.15; // Контур лица и оправа очков (тёмные)
                            }
                        } else if (char === '-') {
                            // Рот или кожа
