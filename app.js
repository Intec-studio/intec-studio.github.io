        ...
        for (let ix = 0; ix < consDots; ix++) {
            for (let iy = 0; iy < consDots; iy++) {
                ...
                // ТВОЙ НОВЫЙ БЛОК:
                    ...
                    // Возвращаем идеальную матрицу 40x26 (все линии очков одинарной толщины)
                    const grid = [
                        "             ##############             ", // 0
                        "          ####------------####          ", // 1
                        "        ###------------------###        ", // 2
                    ...
                    // 1. Считаем плавную амплитуду прыжка
                    let bounceAmplitude = Math.abs(Math.sin(t * 2.5)) * 0.1;
                    
                    // 2. КВАНТУЕМ ШАГ: переводим сдвиг строго в целые строки матрицы
                    let rowOffset = Math.floor(bounceAmplitude / pixelSizeY);
                    
                    // 3. ЖЕСТКАЯ ПРИВЯЗКА: Отнимаем сдвиг ПОСЛЕ округления (никаких pny = ny - yBounce!)
                    // Форма лица блокируется, не растягивается и физически не может "съесть" линию оправы.
                    ...
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        ...
                        if (char === ' ') {
                            ...
                            // Зубы (с 19 по 21 строку)
                            let isTeeth = (row >= 19 && row <= 21) && ((col >= 15 && col <= 17) || (col >= 22 && col <= 24));
                            if (isTeeth) {
                                alpha = 1.0; 
                            } else {
                                alpha = 0.15; // Контур лица и оправа очков
                            }
                        } else if (char === '-') {
                            // Глубина рта
