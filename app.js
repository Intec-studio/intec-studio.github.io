        ...
        for (let ix = 0; ix < consDots; ix++) {
            for (let iy = 0; iy < consDots; iy++) {
                ...
                // ТВОЙ НОВЫЙ БЛОК:
                    ...
                    // Идеально сглаженная матрица 40x29 
                    // (Верх и низ оправы очков утолщены до 2 пикселей для защиты от алиасинга экрана)
                    const grid = [
                        "             ##############             ", // 0
                        "          ####------------####          ", // 1
                        "        ###------------------###        ", // 2
                        ...
                        " ##----------------------------------## ", // 8 (Кожа)
                        " ##----------------------------------## ", // 9 (Кожа - защита)
                        " #################----################# ", // 10 (Оправа верх - слой 1)
                        " #################----################# ", // 11 (Оправа верх - слой 2)
                        " #-----###  ###--######--###  ###-----# ", // 12 (Блик + верх мостика)
                        " #-----###  ###--######--###  ###-----# ", // 13 (Блик + низ мостика)
                        " #-----########--#----#--########-----# ", // 14 (Оправа низ - слой 1)
                        " #-----########--#----#--########-----# ", // 15 (Оправа низ - слой 2)
                        " #------#####----#----#----#####------# ", // 16 (Скругление оправы)
                        "  ##-----------###----###-----------##  ", // 17
                        "  #--###########--------###########--#  ", // 18
                        "  #----------------------------------#  ", // 19
                        "  #----------------------------------#  ", // 20
                    ...
                    // Жесткая анимация по сетке
                    let bounceAmplitude = Math.abs(Math.sin(t * 2.5)) * 0.1; 
                    let rowOffset = Math.floor(bounceAmplitude / pixelSizeY); 
                    
                    // Центрирование (+14 для новой высоты матрицы)
                    let col = Math.floor((nx / pixelSizeX) + 20);
                    let row = Math.floor((ny / pixelSizeY) + 14) - rowOffset; 
                    ...
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        ...
                        if (char === ' ') {
                            // Блики теперь на строках 12 и 13
                            let isGlare = (row === 12 || row === 13) && ((col >= 10 && col <= 11) || (col >= 28 && col <= 29));
                            if (isGlare) {
                                alpha = 1.0; 
                            }
                        } else if (char === '#') {
                            // Зубы теперь на строках 22-24
                            let isTeeth = (row >= 22 && row <= 24) && ((col >= 15 && col <= 17) || (col >= 22 && col <= 24));
                            if (isTeeth) {
                                alpha = 1.0; 
                            } else {
                                alpha = 0.15; 
                            ...
                            // Внутри рта
                            let isInnerMouth = (row >= 22 && row <= 25) && (col >= 13 && col <= 26);
                            if (isInnerMouth) {
                                alpha = 0.25; 
                            } else {
                                alpha = 0.65; 
