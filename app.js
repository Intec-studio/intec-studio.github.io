        ...
        for (let ix = 0; ix < consDots; ix++) {
            for (let iy = 0; iy < consDots; iy++) {
                ...
                // ТВОЙ НОВЫЙ БЛОК:
                    ...
                    // Медленный прыжок (оставляем только Y-смещение)
                    let yBounce = Math.abs(Math.sin(t * 2.5)) * 0.1; 
                    let pnx = nx; // Больше не смещаем вручную, центрируем математикой ниже
                    let pny = ny - yBounce; 
                    
                    // Обновленная матрица 34x25:
                    // Максимально круглая голова, идеально симметричные очки, детализированная рука
                    const grid = [
                        "..................................", // 0
                        ".........############.............", // 1  (Макушка)
                        ".......##YYYYYYYYYYYY##...........", // 2
                        "......#YYYYYYYYYYYYYYYY#..........", // 3
                        ".....#YYYYYYYYYYYYYYYYYY#.........", // 4
                        "....#YYYYY##YYYYYY##YYYYY#........", // 5  (Брови)
                        "....#YYYYY####YYYY####YYYYY#......", // 6
                        "...#YYYYYYYYYYYYYYYYYYYYYY#.......", // 7
                        "..##########################......", // 8  (Верхняя оправа)
                        "..#YY####################YY#......", // 9  (Черные линзы)
                        "..#YY###WW##########WW###YY#......", // 10 (Зрачки - идеально по центру линз)
                        "..#YY###WW##########WW###YY#......", // 11
                        "..#YY####################YY#......", // 12
                        "..##########################..####", // 13 (Нижняя оправа и кончик пальца)
                        "...#YYYYYYYYYYYYYYYYYYYYYY#...#YY#", // 14 (Указательный палец)
                        "....#YYYYYYYYYYYYYYYYYYYY#....#YY#", // 15
                        "....#YYYYYY##########YYYYYY#..#YY#", // 16 (Верх рта)
                        ".....#YYYYY#WWWOOWWW#YYYYY#...#YY#", // 17 (Зубы)
                        ".....#YYYYY#WWWOOWWW#YYYYY#...#YY#", // 18
                        "......#YYYY#OOOOOOOO#YYYY#...##YY#", // 19 (Открытый рот и костяшка пальца)
                        "......#YYYY##########YYYY#...##YY#", // 20 (Низ рта и большой палец)
                        ".......##YYYYYYYYYYYYYY##..#Y##YY#", // 21 (Подбородок и сжатые пальцы)
                        ".........##############....#YYYYY#", // 22 (Нижний контур лица и ладонь)
                        "............................#####.", // 23 (Низ руки)
                        ".................................."  // 24
                    ];
                    ...
                    // Увеличили размер "пикселя" для общего масштаба эмоута
                    let pixelSize = 0.065; 
                    
                    // Математическое центрирование:
                    // Центр головы в нашей текстовой матрице находится на координатах (14.5, 11.5)
                    // Это гарантирует, что само лицо всегда будет ровно по центру квадрата частицы
                    let col = Math.floor((pnx / pixelSize) + 14.5);
                    let row = Math.floor((pny / pixelSize) + 11.5);
                    
                    alpha = 0.0; // По умолчанию делаем прозрачным
                    ...
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        ...
                        if (char === '#') {
                            alpha = 0.1;  // Черный контур и оправа
                        } else if (char === 'Y') {
                            alpha = 0.65; // Желтый (кожа)
                        } else if (char === 'W') {
                            alpha = 1.0;  // Белый (зубы, блики очков)
                        } else if (char === 'O') {
                            alpha = 0.35; // Оранжевый (глубина рта)
                        } else if (char === '.') {
