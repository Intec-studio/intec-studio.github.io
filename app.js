        ...
        for (let ix = 0; ix < consDots; ix++) {
            for (let iy = 0; iy < consDots; iy++) {
                ...
                // ТВОЙ НОВЫЙ БЛОК:
                    ...
                    // Смещение всей группы вправо для идеальной центровки
                    let pnx = nx - 0.16;
                    let pny = ny + 0.1 - yBounce; 
                    
                    // Обновленная матрица: улучшена рука, взгляд прямо, центровка зубов
                    // . - фон, # - черный контур, Y - кожа, W - блики/зубы, O - рот внутри
                    const grid = [
                        ".......##########...........",
                        ".....##YYYYYYYYYY##.........",
                        "....#YYYYYYYYYYYYYY#........",
                        ...
                        "..#YYYYYYYYYYYYYYYYYY#......",
                        ".#######################....",
                        ".#Y###WW##YY###WW##YY##.....", // Зрачки теперь ровно по центру линз
                        "#YY###WW##YY###WW##YYYY#....", // (Взгляд устремлен вперед)
                        "#YY#######YY#######YYYY#....",
                        "#YY#######YY#######YYYY#....",
                        "#YYY#####YYYY#####YYYYY#....",
                        ...
                        ".#YYYYYYYYYYYYYYYYYYYYY#....",
                        "..#YYYYY##########YYYYY#....",
                        "..#YYYYY#WWWOOWWW#YYYYY#....", // Симметричные передние зубы
                        "...#YYYY#WWWOOWWW#YYYY#..##.", // Кончик пальца (толще)
                        "...#YYYY#OOOOOOOO#YYYY#.#YY#", // Указательный палец
                        "....#YYY##########YYY#..#YY#", // Указательный палец
                        ".....##YYYYYYYYYYY##....#YY#", // Указательный палец
                        ".......###########.....##YY#", // Костяшка большого пальца
                        ".....................#YYYYY#", // Основание ладони
                        ".....................#YY#YY#", // Складка сжатых пальцев
                        ".....................#YYYYY#", // Нижняя часть ладони
                        "......................#####.", // Контур низа руки
                        "............................"
                    ];
                    ...
                    // Увеличили размер "пикселя" с 0.048 до 0.058
                    // Это увеличит весь спрайт, и на него придется больше точек холста
                    let pixelSize = 0.058; 
                    
                    // Вычисляем индексы колонки и строки
                    let col = Math.floor((pnx / pixelSize) + (gridWidth / 2));
                    let row = Math.floor((pny / pixelSize) + (gridHeight / 2));
                    ...
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        ...
                        if (char === '#') {
                            alpha = 0.1;  // Черный контур
                        } else if (char === 'Y') {
                            alpha = 0.65; // Желтый (кожа)
                        } else if (char === 'W') {
                            alpha = 1.0;  // Белый (зубы/очки)
                        } else if (char === 'O') {
                            alpha = 0.35; // Оранжевый (внутри рта)
                        } else if (char === '.') {
                            alpha = 0.0;  // Фон (прозрачный)
                        }
                    }
                }
