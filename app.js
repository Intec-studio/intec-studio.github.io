...
// Тут у меня просто склад. Хватаю нужные DOM-элементы один раз при старте 
// и запоминаю текущее состояние (тема, язык, цвет канваса), чтобы потом не гонять 
// document.querySelector по всему коду как не в себя. Это реально спасает производительность ╰(*°▽°*)╯
// currentCanvasColor тяну из CSS-переменной --c-canvas, поэтому обновляю его отдельно 
// в applyTheme с небольшой задержкой, чтобы браузер успел перекрасить рут.
...
// Браузеры - те еще параноики, не дают проигрывать звук, пока я хоть раз не кликну 
// по странице. Это защита от рекламы, обойти нельзя 0_0 Поэтому вешаю "разлочку": 
// первый клик, тап или нажатие клавиши создают AudioContext и снимают блокировку. 
// Как только звук разлочен - слушатели сами себя выпиливают, чтобы не висеть зря :)
...
// Сам шум я генерирую классическим методом Вооса-МакКартни (так варят pink noise 
// в серьезных аудио-библиотеках). Беру чистый белый шум и прогоняю через каскад фильтров, 
// чтобы получить приятный "телевизионный" гул, а не противный писк, от которого кровь из ушей. 
// Чистая математика, никаких тяжелых mp3 файлов! (⌐■_■)
...
// Тут я синхронизирую вообще все: атрибут на <html>, localStorage, активные классы 
// на кнопках-переключателях и плейсхолдеры. Никакой магии, просто делаю так, чтобы 
// после перезагрузки страницы я не оказался внезапно в светлой теме или с другим языком >_<
...
// Мой самодельный SPA-роутер на #hash. Никаких тяжелых фреймворков - просто подменяю 
// страницу через innerHTML по ключу. Дешево и сердито ¯\_(ツ)_/¯
...
window.addEventListener('hashchange', () => {
    ...
    // Отдельная головная боль была с кнопкой "Назад" для статей. Браузер по умолчанию 
    // не умеет по-человечески возвращаться из вложенного состояния. Если просто прогнать 
    // hash через loadPage, страница перерисуется с нуля и скролл подпрыгнет (╯°□°）╯︵ ┻━┻
    // Поэтому я вкатил этот костыль: он перехватывает переход #article -> #patchnotes 
    // и просто скрывает статью на месте, не перерисовывая весь список.
    if (hash === 'patchnotes' && currentHash === 'article') {
        ...
        if (singlePostView && singlePostView.style.display === 'flex') {
            // Просто скрываю статью, не перерисовывая страницу заново
            ...
            updateBreadcrumbsTitle();
            setPanelOffset(0);
            return; // Прерываю работу роутера
        }
    }

    // Если я только что кликнул на статью
    if (hash === 'article') {
        if (document.getElementById('postsList')) {
            // Я перешел из списка, просто запоминаю состояние
            currentHash = 'article';
            return; 
        } else {
            // Срабатывает, если вернулся назад с другой вкладки (например с #rl)
            // Просто кидаю обратно в список статей, чтобы ничего не сломалось
...
// Полностью своя система скролла вместо нативной. Зачем? Потому что мне нужен 
// тонкий кастомный скроллбар и автоскролл колесиком мыши (зажал среднюю кнопку - и едешь, 
// прямо как в винде). Плюс отдельная плавная инерция для тачскринов: свайпнул пальцем - 
// страница едет дальше с затуханием (коэффициент трения 0.92 за кадр), как будто ее 
// реально толкнули физически, а не сухо телепортировали =)
...
function stopAutoScroll() {
    ...
    document.body.classList.remove('autoscroll-active'); // Возвращаю обычную мышку
...
// 1. Блокирую появление уродливого стандарта браузера
...
mainClip.addEventListener('pointerdown', e => {
    // Если я уже автоскроллю, и нажал левую/правую кнопку - отключаю
    ...
    if (e.button === 1) {
        ...
        // Включаю системный курсор (ns-resize)
...
mainClip.addEventListener('wheel', e => { 
    e.preventDefault(); 
    if (isAutoScrolling) return; // Блокирую скролл колесиком, если работает автоскролл
...
mainClip.addEventListener('pointerdown', e => {
    // Игнорирую мышь (оставляю ей колесо), а также инпуты и кнопки
...
window.addEventListener('pointermove', e => { 
    ...
    if (activeDrag === 'thumb') {
        ...
        e.preventDefault();
        const now = Date.now();
        // Считаю скорость движения пальца для инерции
        touchVelocity = (e.pageY - lastTouchY) / (now - lastTouchTime || 1);
        lastTouchY = e.pageY;
        lastTouchTime = now;
        
        // Смещаю панель синхронно с пальцем
...
window.addEventListener('pointerup', e => { 
    if (activeDrag === 'thumb') {
        ...
        // Запускаю плавную инерцию скролла
        let v = touchVelocity * 15; 
        const step = () => {
            if (Math.abs(v) < 0.5) return; 
            let prevOffset = panelOffset; 
            setPanelOffset(panelOffset - v);
            if (panelOffset === prevOffset) { v = 0; return; } // Уперся в конец страницы
...
// Мои "мини-консоли" - декоративные точечные анимации (шторм, радар и т.д.). 
// Каждый тип рисую чистой математикой покадрово, без единой картинки. 
// Считаю расстояние от центра, накидываю шум и синус, и решаю - светить точке в этом 
// кадре или нет. Дешево для видеокарты и ноль байт веса! (*^▽^*)
// 
// Но главное - IntersectionObserver. Он следит, какие канвасы видны на экране прямо сейчас. 
// Анимировать то, что скрыто за пределами вьюпорта - это просто убивать FPS на слабых ПК. 
// Поэтому невидимые канвасы тупо ставятся на паузу. Оптимизация в абсолют! B-)
let time = 0;
let consData = [];
            ...
            for (let iy = 0; iy < consDots; iy++) {
                ...
                if (c.type === 'storm') { 
                    ...
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        ...
                        if (char === ' ') {
                            // ИСПРАВЛЕНО: проверяю, что пробелы не только на 10-11 строках, 
                            // но и находятся внутри лица (отсекаю колонки 0 и 39)
                            ...
                            // Проверяю координаты зубов по матрице
...
// Тут вся логика раздела с патчнотами: поиск, фильтр по категориям и сортировка.
// Отдельный кайф - рендер "полной" статьи с аудиоплеером внутри. 
// Плеер я собрал полностью кастомный: play/pause, перемотка, громкость. И самое важное - 
// привязал обновление ползунка к requestAnimationFrame (60 раз в секунду), чтобы полоска 
// ехала плавно и без дерганий. Все на чистом JS, без сторонних библиотек (⌐■_■)
function initPatchnotesUI() {
    ...
    function renderPostsList() {
        ...
        let filtered = (typeof POSTS_DATABASE !== 'undefined' ? POSTS_DATABASE : []).filter(post => {
            // Безопасно достаю описание (если его вдруг нет, беру пустую строку)
    ...
    async function openPost(post) {
        ...
        try {
            ...
            initCanvases(); // Вычищаю память даже если произошла ошибка
        }
    }

    if (backToPostsBtn) {
        backToPostsBtn.addEventListener('click', () => {
            // --- 2. МЕНЯЮ ЛОГИКУ КНОПКИ ВНУТРИ СТАТЬИ ---
            window.history.back();
        });
    }
...
// Имитация старого телевизора: плейлист видео, статика между переключением
// каналов (та самая функция playStaticNoiseSound сверху) и часы вместо программы
// передач, если плейлист пуст. Если захочу добавить свои видео - мне нужно просто 
// закинуть ссылки строкой в playlist ниже (главное не забыть "" для ссылок, 
// а то все к чертям сломается X_X).
const appRLTV = {
    // МАССИВ ССЫЛОК НА ВИДЕО. (надо не забыть "" для ссылок!)
    playlist: [
    ...
    syncVol(source) {
        ...
        // Закрашиваю канавку слайдера фирменным цветом
...
document.getElementById('globalBackBtn').addEventListener('click', () => {
    ...
    // Если я внутри статьи - эмулирую системную кнопку "Назад"
    if (singlePostView && singlePostView.style.display === 'flex') {
        window.history.back();
    } else {
        // Иначе выхожу на стартовый экран
