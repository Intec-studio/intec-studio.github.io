        ...
        for (let ix = 0; ix < consDots; ix++) {
            for (let iy = 0; iy < consDots; iy++) {
                ...
                // ТВОЙ НОВЫЙ БЛОК:
                    ...
                    // Финальная матрица 40x26 (без острых уголков по бокам)
                    const grid = [
                        "          ####################          ", // 0
                        "        ##--------------------##        ", // 1
                        "      ###----------------------###      ", // 2
                    ...
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        ...
                        if (char === ' ') {
                            // СТРОГАЯ ПРОВЕРКА: блики (пробелы) могут быть ТОЛЬКО внутри очков
                            let isGlare = (row === 10 || row === 11) && ((col >= 10 && col <= 11) || (col >= 28 && col <= 29));
                            if (isGlare) {
                                alpha = 1.0; // Глаза (ярко-белые)
                            }
                            // Остальные пробелы игнорируются (остается alpha = 0.0)
                        } else if (char === '#') {
                            // Зубы или контур
