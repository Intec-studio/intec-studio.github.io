        ...
        for (let ix = 0; ix < consDots; ix++) {
            for (let iy = 0; iy < consDots; iy++) {
                ...
                // ТВОЙ НОВЫЙ БЛОК:
                    ...
                    alpha = 0.0; // Пустота по умолчанию
                    
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        let char = grid[row][col];
                        
                        if (char === '#') {
                            alpha = 1.0;  // Белые точки (контур и детали)
                        } else if (char === '-' || char === ' ') {
                            // Если это не пробел (пустота снаружи), а символ внутри
                            // Проверяем, не является ли пробел частью "глаз" или "рта"
                            if (char === '-') {
                                alpha = 0.3; // Серые точки (кожа)
                            } else {
                                // Пробелы внутри очков/глаз тоже делаем серыми или оставляем пустыми
                                // Чтобы глаза были "пустыми", оставляем 0.0
                                // Но судя по рисунку, пробелы в глазах — это пустота
                                alpha = 0.0; 
                            }
                        }
                    }
                    
                    // Дополнительная проверка для пробелов внутри лица (зубы, глаза)
                    // Если символ ' ' находится внутри очков, можно дать ему alpha 1.0
                    // В твоем арте глаза и зубы — это пробелы:
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        let char = grid[row][col];
                        if (char === ' ') {
                            // Координаты зубов и глаз по твоей сетке
                            let isEye = (row >= 10 && row <= 12);
                            let isTooth = (row >= 19 && row <= 22);
                            if (isEye || isTooth) {
                                // Проверяем, что мы не в "ушах" или за пределами лица
                                if (col > 10 && col < 30) {
                                     alpha = 1.0; // Делаем зубы и глаза белыми
                                }
                            }
                        }
                    }
