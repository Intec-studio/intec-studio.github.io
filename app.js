        ...
        for (let ix = 0; ix < consDots; ix++) {
            for (let iy = 0; iy < consDots; iy++) {
                ...
                // ТВОЙ НОВЫЙ БЛОК:
                    ...
                    // Матрица пиксель-арта строго по вашей картинке (28 ширина, 26 высота)
                    // . - фон, # - черный, Y - желтый, W - белый, O - оранжевый
                    const grid = [
                        ".......##########...........",
                        ".....##YYYYYYYYYY##.........",
                        "....#YYYYYYYYYYYYYY#........",
                    ...
                    let gridWidth = 28;
                    let gridHeight = 26;
                    // Размер одного "пикселя" в системе координат частиц (настроен так, 
                    // чтобы размер головы совпадал с вашим предыдущим радиусом 0.65)
                    let pixelSize = 0.048; 
                    
                    // Переводим непрерывные координаты частиц в дискретные индексы сетки (колонки и строки)
                    // (Предполагается, что центр координат частиц (0,0) находится в центре сетки)
                    ...
                    if (col >= 0 && col < gridWidth && row >= 0 && row < gridHeight) {
                        ...
                        if (char === '#') {
                            alpha = 0.1;  // Черный контур (очень тусклая точка)
                        } else if (char === 'Y') {
                            alpha = 0.65; // Желтый (светло-серая точка)
                        } else if (char === 'W') {
                            alpha = 1.0;  // Белый (максимально яркая точка)
                        } else if (char === 'O') {
                            alpha = 0.35; // Оранжевый (темно-серая точка)
                        } else if (char === '.') {
